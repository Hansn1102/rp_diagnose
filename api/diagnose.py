"""Vercel Python 서버리스 함수: /api/diagnose?keyword=...

데이터랩(검색어트렌드) + 검색광고(키워드도구) 두 API만으로
키워드 수요·기회 진단 JSON을 반환한다. 표준 라이브러리만 사용(의존성 0).

환경변수(Vercel 대시보드 Settings → Environment Variables):
  NAVER_CLIENT_ID, NAVER_CLIENT_SECRET           (데이터랩)
  NAVER_AD_ACCESS_LICENSE, NAVER_AD_SECRET_KEY, NAVER_AD_CUSTOMER_ID  (검색광고)
"""
...
def fetch_keywords(keyword):
    lic = os.environ["NAVER_AD_ACCESS_LICENSE"]
    sec = os.environ["NAVER_AD_SECRET_KEY"]
    cust = str(os.environ["NAVER_AD_CUSTOMER_ID"])
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "X-API-KEY": lic,
            "X-Customer": cust,
            "X-Signature": _ad_sig(ts, method, path, sec),
        },
        method="GET",
    )
    ...
    out = []
    for x in rows:
        pc = _to_int(x.get("monthlyPcQcCnt"))
        mo = _to_int(x.get("monthlyMobileQcCnt"))
        out.append({
            "keyword": x.get("relKeyword", ""),
            "pc": pc, "mobile": mo, "monthly": pc + mo,
            "comp": x.get("compIdx", ""),
        })
...
def diagnose(keyword):
    ...
    return {
        "keyword": keyword,
        "monthly_total": exact["monthly"],
        "monthly_pc": exact["pc"],
        "monthly_mobile": exact["mobile"],
        "comp_idx": exact["comp"],
        "growth_pct": growth,
        "device_split": device,
        "trend": trend,
